...
async fn process_collect_article_links<H: HttpClient>(
    client: &H,
    feeds: &[Feed],
    pool: &PgPool,
) -> Result<()> {
...
async fn process_collect_articles<F: FirecrawlClient>(
    firecrawl_client: &F,
    pool: &PgPool,
) -> Result<()> {
    println!("--- 記事内容取得開始 ---");
...
mod tests {
    ...
    mod unit {
        ...
        #[sqlx::test(fixtures("../../sql/fixtures/workflow/basic.sql"))]
        ...
        #[sqlx::test(fixtures("../../sql/fixtures/workflow/mixed.sql"))]
