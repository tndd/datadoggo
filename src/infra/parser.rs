...
mod tests {
    ...
    fn test_parse_common_date_formats() {
        ...
        assert_eq!(parse_date_string(rfc3339).unwrap(), expected_rfc3339);

        // RFC 2822 (RSSで一般的)
        let rfc2822 = "Sun, 10 Aug 2025 12:30:00 +0000";
        assert_eq!(parse_date_string(rfc2822).unwrap(), expected_rfc3339);

        // YYYY-MM-DD（dateparserは現在時刻で補完するため、日付のみをチェック）
        let ymd = "2025-08-10";
        let parsed_ymd = parse_date_string(ymd).unwrap();
        assert_eq!(
    ...
    fn test_parse_with_timezones() {
        ...
        assert_eq!(parse_date_string(jst_str).unwrap(), expected_utc);

        // PST (-08:00)
        let pst_str = "2025-08-10T04:30:00-08:00";
        assert_eq!(parse_date_string(pst_str).unwrap(), expected_utc);
    }
    ...
    fn test_parse_invalid_formats() {
        ...
        assert!(parse_date_string("2025-13-40").is_err()); // 不正な月日
        assert!(parse_date_string("").is_err()); // 空文字列
    }
    ...
    fn test_parse_from_mock_rss_files() {
        ...
        assert_eq!(parse_date_string(bbc_date).unwrap(), expected_bbc);
        ...
        assert_eq!(parse_date_string(cbs_date).unwrap(), expected_cbs);
        ...
        assert_eq!(parse_date_string(guardian_date).unwrap(), expected_guardian);
        ...
        for (date_str, expected) in &rfc2822_variations {
            assert_eq!(
                parse_date_string(date_str).unwrap(),
                *expected,
                "日付文字列 '{}' のパースが期待と異なります",
                date_str
            );
